...
const ActivityItem = ({ activity }: { activity: DashboardData['recentActivity'][0] }) => {
  const getActivityIcon = (action: string) => {
    switch (action) {
      ...
      case 'studied': return <BookOpen className="w-4 h-4 text-blue-500" />
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mt-1">
          <span className="text-xs text-gray-500 dark:text-gray-400">
            {formatTime(activity.timeSpent)} • {Math.round(activity.masteryScore * 100)}% mastery
          </span>
...
const AchievementItem = ({ achievement }: { achievement: DashboardData['achievements'][0] }) => {
  const getAchievementIcon = (type: string) => {
    switch (type) {
      ...
      case 'mastery': return <Brain className="w-5 h-5 text-purple-500" />
...
export default function Dashboard() {
    ...
    const fetchEmergencyCount = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setEmergencyCount(pendingCount)
        }
      } catch (err) {
  ...
  const getPerformanceTrend = (current: number, previous: number) => {
    if (current > previous) return { trend: 'up', percentage: Math.round(((current - previous) / previous) * 100) }
    if (current < previous) return { trend: 'down', percentage: Math.round(((previous - current) / previous) * 100) }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Button
              ...
              onClick={() => window.open('/admin/emergency-contacts', '_blank')}
            >
